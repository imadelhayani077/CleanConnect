        ...
        switch (activePage) {
            case "book-new":
                return (
                    <Booking
                        onSuccess={() => {
                            // Ideally, you should use a router navigation here instead of window.location
                            // but this works for a hard refresh redirect
                            window.location.href = "/dashboard?tab=my-bookings";
                        }}
                    />
                );
            case "my-bookings":
                        ...
                        <div className="relative overflow-hidden bg-primary/5 p-8 md:p-10 rounded-3xl border border-primary/10">
                            <div className="relative z-10 max-w-2xl">
                                ...
                                <p className="text-muted-foreground mt-4 text-lg leading-relaxed">
                                    Your home deserves to shine. Ready to book
                                    your next professional cleaning session?
                                </p>

                                <div className="mt-8 flex flex-wrap gap-4">
                                    {/* Note: In a real app, this button should likely toggle the activePage state via a parent handler */}
                                    <button className="flex items-center gap-2 px-6 py-3 bg-primary text-primary-foreground font-semibold rounded-lg shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300">
                                        Book a Cleaning
